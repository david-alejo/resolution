...
namespace simulator {
	...
	const double ControllerSimpleUAV::defaultGoalDistance = 0.0;
	const double ControllerSimpleUAV::WP_TOO_NEAR = 0.1; // in meters

ControllerSimpleUAV::ControllerSimpleUAV(){
  pointersToNull();
  init();
}

void ControllerSimpleUAV::pointersToNull(){
  planeVec = NULL;
}

ControllerSimpleUAV * ControllerSimpleUAV::createFromFile(const std::string& fileName) const {
  //! \Note Dummy
  ControllerSimpleUAV *ret = NULL;
	...
	try {
		...
		std::cout << "ControllerSimpleUAV::createFromFile --> Error while loading data from file: ";
...
ControllerSimpleUAV* ControllerSimpleUAV::createFromBlock(ParseBlock& data) const
{
	ControllerSimpleUAV *ret = new ControllerSimpleUAV();
	...
	try {	
		...
		std::cout << "ControllerSimpleUAV::createFromBlock --> Error while getting data from block: ";
		std::cout << e.what() << std::endl;
		throw(e);
	}
...
Checker *ControllerSimpleUAV::controllerFileChecker() const {
  //! \Note Dummy
...
ControllerSimpleUAV *ControllerSimpleUAV::create(const std::vector <double> &def){
  if(def.size() % 3 != 1){
    throw (std::runtime_error("ControllerSimpleUAV creation exception"));
  }
...
void ControllerSimpleUAV::init(const FlightPlan& wpl, const std::vector<double> &initial_state)
{
  init();
  fp = wpl;
...
void ControllerSimpleUAV::dispose () throw () {
  fp.clear();
  delete planeVec;
  init();
	controlVector.clear();
...
ControllerSimpleUAV *ControllerSimpleUAV::clone () const { 
  ControllerSimpleUAV *ret= new ControllerSimpleUAV();
  ret->speed = speed;
...
std::string ControllerSimpleUAV::toString() const {
  using std::endl;
...
void ControllerSimpleUAV::update(const std::vector< double >& _parameter, const std::vector< double >& _state, const std::vector< double >& _control){
...
void ControllerSimpleUAV::update(){
...
void ControllerSimpleUAV::update_parameter(){
  parameterVector [0] = speed;
}


void ControllerSimpleUAV::update_control(){
    //! Here comes the control logic!
//    Control: [ c_phi c_h ]
      controlVector[0] = speed;
...
bool ControllerSimpleUAV::update_nextWpIndex(){
...
double ControllerSimpleUAV::updateWaypoint(bool vertical_plane) {
  Point3D curPoint(stateVector[0], stateVector[1], stateVector[2]);
...
//       cout << "Next wp dist." << next_wp_dist <<"\n";
  ...
  void ControllerSimpleUAV::setFlightPlan(const FlightPlan &nwfp){
		delete planeVec;
		pointersToNull();
		init(nwfp, stateVector);
	}
	
FlightPlan ControllerSimpleUAV::getFlightPlan() const
{
	return fp;
}

int ControllerSimpleUAV::checkPlanFeasibility(const simulator::FlightPlan& _plan) const
{
	int ret = 0;
...
int ControllerSimpleUAV::checkPlanFeasibility() const
{
	return checkPlanFeasibility(fp);
}

int ControllerSimpleUAV::checkTwoWpFeasibility(const functions::Point3D& a, const functions::Point3D& b) const
{
...
ParseBlock* ControllerSimpleUAV::toBlock()
{
