#ifndef __CONTROLLER_QUAD_CATEC_H__
#define __CONTROLLER_QUAD_CATEC_H__
 ...
 */
class ControllerQuadCatec : public Controller{
public:
  ...
  */
  ControllerQuadCatec *createFromFile(const std::string &fileName)const;
  
  /*! Creation from parse block.
  
  */
  ControllerQuadCatec *createFromBlock(ParseBlock &fileName)const;
  
  //!Destructor
  ~ControllerQuadCatec();
  
  //! Only clone is allowed
  ControllerQuadCatec * clone () const;  
  ...
  virtual std::string getType() const { return "ControllerQuadCatec"; };
...
protected:
  ...
  ControllerQuadCatec (const ControllerQuadCatec &that);
  //!@brief Hidden assignment operator (use clone always)
  ControllerQuadCatec & operator = (const ControllerQuadCatec &that);
  //!@brief Sets all pointers to Null
  void pointersToNull();
  
  virtual void init ( const ControllerQuadCatec &other);
