...
{
	double longitude_shift = equatorial_shift * cos(lat);
...
functions::RealVector EarthLocation::toRelative(const EarthLocation& center) const
{
  functions::RealVector ret;
  double inc_east = (lon - center.lon) * equatorial_shift * cos( (center.lat + lat) / 2);
