...
if [ $# -ne 10 ]; then
  echo "Usage: $0 <min_number_UAVs> <max_UAVs> <path to the repository module> <number of test with each UAV number> <base_input_file> <base_output_file> <population> <generations> <cruise_speed> <cost_type>"
  exit 1
fi
...
#     echo  Call : $3/build/edit_test $5$CONTADOR2 --output $6$CONTADOR2 --population $7 --generations $8 --cost_type ${10}
    $3/build/edit_test $5$CONTADOR2 --output $6$CONTADOR2 --population $7 --generations $8 --cost_type ${10}
    echo Substituting the cruise_speed
    let CONTADOR2+=1
  done
