...
for curr_method=1:length(data)
  for m=1:length(data{curr_method}{n}{2}) % For each experiment
    cont_t = 1;
    last_cost = max_cost;
    curr_time = data{curr_method}{n}{2}(1)
    last_time = 0;
    cont_it = 1;
    for t = delta_t:delta_t:final_time
      % First get the first value above
      while (data{curr_method}{n}{2}(cont_it) < t)
        last_cost = data{curr_method}{n}{1}(cont_it);
        last_time = data{curr_method}{n}{2}(cont_it);
        cont_it = cont_it + 1;
      end
      % Get the time and cost of the current data used
      curr_time = data{curr_method}{n}{2}(cont_it);
      curr_cost = data{curr_method}{n}{1}(cont_it);
      % Interpolation
      cost{curr_method}(m,cont) = last_cost + (curr_cost - last_cost)/(curr_time - last_time) * (t - last_time);
    end
  end
  median_cost{curr_method}=median(cost{curr_method});
 
  
  plot(time_vector, median_time{curr_method}{2}, color(curr_method), 'lineWidth', 3);
  hold on;
end

setLabelStyle('Number of iterations', 'Median of the cost(s)');
...
%      fig4_data{curr_method}{1}(i+1)=median_time{curr_method}{2}(i); % The time in seconds
%    end
