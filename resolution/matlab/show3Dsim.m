function show3Dsim(data,geom,name)
...
for n=1:n_uavs
    for k=1:size(geom,1)
        [X Y Z]=boxFromLoHi(data{n}(1,1)-semigeom(k,1), ...
                        data{n}(1,2)-semigeom(k,2), ...
                        data{n}(1,3)-semigeom(k,3), ...
                        data{n}(1,1)+semigeom(k,1), ...
                        data{n}(1,2)+semigeom(k,2), ...
                        data{n}(1,3)+semigeom(k,3));
...
while ~quit_prog
    ...
    if pause_mode==0
        t=t+1;
	curr_name = [name num2str(t) '.jpg']
	saveas(h, curr_name, 'jpg')
    end
    pause(time_lap);
end
return;
        ...
        for nn=1:n_uavs
            if c_time <=size(data{nn},1)
                for kk=1:size(geom,1)
                    [X Y Z]=boxFromLoHi(data{nn}(c_time,1)-semigeom(kk,1), ...
                        data{nn}(c_time,2)-semigeom(kk,2), ...
                        data{nn}(c_time,3)-semigeom(kk,3), ...
                        data{nn}(c_time,1)+semigeom(kk,1), ...
                        data{nn}(c_time,2)+semigeom(kk,2), ...
                        data{nn}(c_time,3)+semigeom(kk,3));
